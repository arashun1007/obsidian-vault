...
let rankView = null; // null | {loading:1} | {error:msg} | {data, alert}
let rankTab = "score"; // "score" | "fish"
let rankPeriod = "week"; // "week"（今週の部） | "all"（殿堂）
let weeklySent = loadJSON("isomoguri_wsent", { week: 0, s: 0, c: 0 }); // 今週送信済みのベスト
let runBig = { cm: 0, id: "" }; // このランで一番大きかった魚
const SHARE_URL = "https://isomoguri.arai10070223.workers.dev/";

// サーバーと同じ週番号（JST・月曜はじまり）
function clientWeek(now = Date.now()) {
...
function postScore(run, runCm, runFish) {
  const payload = {
  ...
  fetch("api/score", {
    ...
    if (res && res.ok) {
      sentBest = { s: highScore, c: bigFish.cm, n: playerName, v: titleIdx() };
      saveJSON("isomoguri_sent", sentBest);
      const wk = clientWeek();
      if (weeklySent.week !== wk) weeklySent = { week: wk, s: 0, c: 0 };
      weeklySent.s = Math.max(weeklySent.s, run);
      weeklySent.c = Math.max(weeklySent.c, runCm);
      saveJSON("isomoguri_wsent", weeklySent);
    }
  }).catch(() => {});
}

// 名前登録・変更時：通算ベストだけ同期する
function submitScore() {
  if (!NET_OK || !playerName) return;
  if (highScore <= sentBest.s && bigFish.cm <= sentBest.c && playerName === sentBest.n &&
      titleIdx() <= (sentBest.v || 0)) return;
  postScore(0, 0, "");
}

// ラン終了時：今週の部にはそのランの成績を送る
function submitRun(total) {
  if (!NET_OK || !playerName) return;
  const wk = clientWeek();
  if (weeklySent.week !== wk) weeklySent = { week: wk, s: 0, c: 0 };
  const improved = total > weeklySent.s || runBig.cm > weeklySent.c ||
    highScore > sentBest.s || bigFish.cm > sentBest.c || playerName !== sentBest.n ||
    titleIdx() > (sentBest.v || 0); // 称号が上がったら送る
  if (!improved) return;
  postScore(Math.max(0, total), runBig.cm, runBig.id);
}

// 前回見たときの自分の順位と比べ、抜かれていたら知らせる（今週の部で判定）
function rankAlert(d) {
  const w = d.weekly || {};
  const cur = {
    week: w.week || 0,
    s: (w.me && w.me.score) || 0,
  ...
  const prev = loadJSON("isomoguri_lastrank", null);
  saveJSON("isomoguri_lastrank", cur);
  if (!prev || !playerName || prev.week !== cur.week) return null;
  const drops = [];
  if (prev.s && cur.s > prev.s) {
    const culprit = (w.score || [])[cur.s - 2];
    drops.push(`スコア${prev.s}位→${cur.s}位` + (culprit ? `（${culprit.n}に抜かれた）` : ""));
  }
  if (prev.f && cur.f > prev.f) {
    const culprit = (w.fish || [])[cur.f - 2];
    drops.push(`ヌシ${prev.f}位→${cur.f}位` + (culprit ? `（${culprit.n}に抜かれた）` : ""));
  }
  return drops.length ? "⚠ " + drops.join("　") : null;
}
...
function fetchRanking() {
  ...
  fetch("api/ranking?id=" + playerId())
    ...
    .then(d => { rankView = { data: d, alert: rankAlert(d) }; })
    .catch(e => {
...
function buildShareCard() {
  const S = 4, W = 240, H = 126; // 論理240x126をx4で出力（960x504）
  ...
  gr.addColorStop(0, "#0a3c64"); gr.addColorStop(1, "#04121f");
  g.fillStyle = gr; g.fillRect(0, 0, W, H);
  g.fillStyle = "rgba(200,230,255,0.06)";
  for (const [x0, w] of [[30, 18], [70, 10], [150, 22], [205, 12]]) {
    g.beginPath();
    g.moveTo(x0, 0); g.lineTo(x0 + w, 0); g.lineTo(x0 + w - 28, H); g.lineTo(x0 - 28, H);
    g.closePath(); g.fill();
  }
  g.strokeStyle = "rgba(255,255,255,0.25)";
  for (const [bx, by, br] of [[20, 100, 2], [50, 40, 3], [200, 80, 2], [225, 30, 3], [120, 110, 2]]) {
  ...
  if (best) {
    g.save();
    g.translate(cx, cy);
    g.fillStyle = "rgba(255,224,102,0.10)";
    for (let i = 0; i < 8; i++) {
      g.rotate(Math.PI / 4);
      g.beginPath(); g.moveTo(0, 0); g.lineTo(52, -9); g.lineTo(52, 9); g.closePath(); g.fill();
    }
    g.restore();
    const img = sprites[best.id + "_big"] || sprites[best.id];
    if (img && img.width) {
      const w = Math.min(84, 56 * best.sp.size * (best.nushi ? 1.25 : 1));
    ...
    if (img && img.width) {
      const w = 56, h = img.height / img.width * w;
  ...
  if (best) {
    ...
    g.fillText("ボウズ。海はそういう日もある。", tx, 66);
  }
  g.font = "bold 10px monospace"; g.fillStyle = "#cfd8e3";
  g.fillText("スコア " + (resultData ? resultData.total : 0) + "pt　漁果 " + (resultData ? resultData.rank : "-"), tx, 84);
  g.font = "bold 9px monospace"; g.fillStyle = "#7f9db8";
  g.fillText(localDateKey().replace(/-/g, "/") + "　海況 " + mode.name, tx, 100);
  g.font = "bold 7px monospace"; g.fillStyle = "#5a7a94";
  g.textAlign = "center";
  g.fillText(SHARE_URL.replace("https://", "").replace(/\/$/, ""), W / 2, 118);
...
// LINE等アプリ内ブラウザは <a download> を「このブラウザでは保存できません」と拒否することがある。
// window.openの新規タブもポップアップとして塞がれがちなので、画像を直接ページ上に表示して
// 「長押しで保存」してもらう方式にする。これは共有APIが無い環境でも確実に効く。
function showShareImage(dataUrl) {
  const wrap = document.createElement("div");
  wrap.style.cssText = "position:fixed;inset:0;background:rgba(4,18,34,0.94);z-index:9999;" +
    "display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:16px;" +
    "font-family:monospace;";
  const img = document.createElement("img");
  img.src = dataUrl;
  img.style.cssText = "max-width:92vw;max-height:68vh;border:2px solid #ffe066;border-radius:4px;";
  const msg = document.createElement("div");
  msg.textContent = "画像を長押しして「写真に保存」してください";
  msg.style.cssText = "color:#ffe066;font-weight:bold;font-size:13px;text-align:center;";
  const closeBtn = document.createElement("button");
  closeBtn.textContent = "閉じる";
  closeBtn.style.cssText = "padding:8px 28px;background:#1e3a52;color:#9ad8f0;" +
    "border:1px solid #9ad8f0;border-radius:4px;font-family:monospace;font-size:13px;";
  closeBtn.onclick = () => wrap.remove();
  wrap.appendChild(img); wrap.appendChild(msg); wrap.appendChild(closeBtn);
...
function shareResult() {
  ...
  let cnv;
  try { cnv = buildShareCard(); } catch (_) { return; }
  const best = bestCatch;
  const text = "【いそもぐり】" + (playerName ? playerName + "が" : "") +
    (best ? (best.nushi ? "ヌシ・" : "") + best.sp.name + " " + best.cm + "cmをキープ！" : "潜ってきた。") +
    " スコア" + resultData.total + "pt";
  if (!cnv.toBlob) { showShareImage(cnv.toDataURL()); return; }
  cnv.toBlob(blob => {
    if (!blob) { showShareImage(cnv.toDataURL()); return; }
    const file = typeof File === "function" ? new File([blob], "isomoguri.png", { type: "image/png" }) : null;
    if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
      navigator.share({ files: [file], text: text + " " + SHARE_URL }).catch(err => {
        if (err && err.name === "AbortError") return; // ユーザーが共有をキャンセルしただけ
        showShareImage(cnv.toDataURL());
      });
    } else {
      // navigator.share非対応、または画像共有非対応（LINE等のアプリ内ブラウザ）
      showShareImage(cnv.toDataURL());
    }
  }, "image/png");
}
...
let calmT = 0;                                        // 静まり：キー発火後は心拍が落ち、酸素消費が半分になる
...
function reset() {
  ...
  runCounts = {}; firedTriggers = {}; calmT = 0;
  spawnTerrain();
  spawnFish();
  spawnHazards();
}
...
function onUp(e) {
  ...
  if (g.startedState === "rank" && state === "rank" && isTap) {
    // 期間タブ（今週の部／殿堂）
    if (g.y >= 38 && g.y <= 58) {
      if (g.x < VW / 2 && rankPeriod !== "week") { rankPeriod = "week"; sfx.ui(); }
      else if (g.x >= VW / 2 && rankPeriod !== "all") { rankPeriod = "all"; sfx.ui(); }
      return;
    }
    // 部門タブ（スコアの部／ヌシの部）
    if (g.y >= 62 && g.y <= 82) {
  ...
  if (g.startedState === "result" && state === "result" && isTap && performance.now() - resultData.t > 800) {
    if (g.x >= VW - 62 && g.y < 32) { sfx.ui(); shareResult(); return; }
...
// タブ切替・アプリ背面化・ホーム画面へ移動したときはBGMを止める。
// unlockElはiOSのマナーモード対策でわざと鳴らし続ける無音ループのため、
// これを止めない限りバックグラウンドでもBGMのオーディオセッションが生き続けてしまう。
document.addEventListener("visibilitychange", () => {
  if (!document.hidden) return;
  if (AC && AC.state === "running") AC.suspend();
  if (unlockEl && !unlockEl.paused) unlockEl.pause();
});
...
function catchFish(f, quality) {
  ...
  if (f.sp.pts > 0) {
    ...
    // 生涯最大の1匹（ランキング「ヌシの部」用）と、このラン最大の1匹（今週の部用）
    if (f.cm > bigFish.cm) {
      bigFish = { cm: f.cm, id: f.id };
      saveJSON("isomoguri_bigfish", bigFish);
    }
    if (f.cm > runBig.cm) runBig = { cm: f.cm, id: f.id };
  } else {
    breakCombo();
  }
...
function update(dt) {
  ...
  if (state === "dive" || state === "aim" || state === "struggle") {
    ...
    const calmMul = calmT > 0 ? 0.5 : 1; // 静まり中は心拍が低い
    oxygen -= dt * (0.6 + depthM / 14) * mode.o2 * drainMul * calmMul;
...
function drawHUD() {
  ...
  ctx.fillStyle = r < 0.3 ? (Math.sin(tGame * 8) > 0 ? "#ff5c5c" : "#a03030")
                : calmT > 0 ? "#7bd88f" : "#4dd2ff";
  ctx.fillRect(8, 8, 102 * Math.max(0, r), 8);
  ctx.font = "bold 8px monospace"; ctx.textAlign = "left";
  ctx.fillStyle = "#fff"; ctx.fillText("O2", 10, 15);
  if (calmT > 0) { ctx.fillStyle = "#7bd88f"; ctx.fillText("静", 116, 15); }
  // 深度
  ctx.textAlign = "right";
...
function drawTitle() {
  ...
  ctx.fillStyle = "#ffe066";
  ctx.fillText("ハイスコア " + highScore + "pt", VW / 2, 116);
  ctx.font = "bold 8px monospace";
  const nextTtl = TITLES[titleIdx() + 1];
  const ttlLine = nextTtl
    ? `称号 ${TITLES[titleIdx()].name}（次の${nextTtl.name}まであと${nextTtl.need - (progress.totalScore || 0)}pt）`
    : `称号 ${TITLES[titleIdx()].name}（最高位）`;
  ctx.fillStyle = TITLES[titleIdx()].color;
  ctx.fillText(ttlLine, VW / 2, 130);
  ctx.fillStyle = "#9ab0c4";
  ctx.fillText(`潜水 ${progress.dives}　連続 ${progress.streak}日　最多 ${progress.bestCombo}連`, VW / 2, 142);
...
function drawResult() {
  ...
  // 自慢カード（シェア）ボタン
  ctx.fillStyle = "rgba(20,40,60,0.9)"; ctx.fillRect(VW - 60, 6, 54, 16);
  ctx.strokeStyle = "#ffe066"; ctx.lineWidth = 0.5; ctx.strokeRect(VW - 59.5, 6.5, 53, 15);
  ctx.fillStyle = "#ffe066";
  ctx.fillText("自慢する", VW - 33, 17);
...
function drawRank() {
  ...
  // 期間タブ（今週の部／殿堂）と部門タブ（スコア／ヌシ）の2段
  const drawTabRow = (pairs, sel, top) => {
    ctx.font = "bold 9px monospace";
    pairs.forEach(([id, label], i) => {
      ...
      ctx.fillRect(tx, top, tw, 18);
      ctx.strokeStyle = active ? "#ffe066" : "#40566a"; ctx.lineWidth = active ? 1.5 : 1;
      ctx.strokeRect(tx + 0.5, top + 0.5, tw - 1, 17);
      ctx.fillStyle = active ? "#ffe066" : "#7f9db8";
      ctx.textAlign = "center";
      ctx.fillText(label, tx + tw / 2, top + 13);
    });
  };
  drawTabRow([["week", "今週の部"], ["all", "殿堂（通算）"]], rankPeriod, 40);
  drawTabRow([["score", "スコアの部"], ["fish", "ヌシの部"]], rankTab, 64);

  ctx.font = "bold 8px monospace";
  if (!rankView || rankView.loading) {
    ctx.fillStyle = "#9ad8f0";
    ctx.fillText("取得中…", VW / 2, 110);
  } else if (rankView.error) {
    ctx.fillStyle = "#ff8c8c";
    wrapText(rankView.error, VW / 2, 110, 190, 12);
  } else {
    const d = rankView.data;
    const base = rankPeriod === "week" ? (d.weekly || {}) : d;
    const medal = ["#ffe066", "#cfd8e3", "#e0a06a"];
    const rows = rankTab === "score" ? (base.score || []) : (base.fish || []);
    let y = 100;
    if (rankPeriod === "week") {
      ctx.fillStyle = "#7f9db8"; ctx.textAlign = "center";
      ctx.fillText("毎週月曜0時にリセット", VW / 2, 94);
      y = 108;
    }
    if (!rows.length) {
      ctx.fillStyle = "#7f9db8"; ctx.textAlign = "center";
      ctx.fillText(rankPeriod === "week" ? "今週はまだ誰もいない。一番乗りのチャンス。"
                                         : "まだ誰もいない。", VW / 2, y + 6);
      y += 20;
    }
    ctx.textAlign = "left";
    ...
    ctx.textAlign = "center";
    if (rankView.alert) {
      ctx.globalAlpha = 0.75 + 0.25 * Math.sin(tGame * 5);
      ctx.fillStyle = "#ff8c8c";
      wrapText(rankView.alert, VW / 2, VH - 80, 205, 10);
      ctx.globalAlpha = 1;
    }
    ...
    if (playerName) {
      ...
      const myTitle = TITLES[titleIdx()].name + "・";
      ctx.fillStyle = "#7bd88f";
